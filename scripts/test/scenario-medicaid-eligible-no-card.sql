-- Scenario: Medicaid + signed contract + paid deposit + no card on file
-- Expected API: is_eligible=true (Medicaid does not require card on file)
-- Usage: ./scripts/test/run-portal-readiness-scenario.sh medicaid-eligible-no-card

BEGIN;

UPDATE public.phi_clients
SET payment_method = 'Medicaid',
    updated_at = NOW()
