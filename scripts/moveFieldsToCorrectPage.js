...
async function moveFieldsToCorrectPage() {
  try {
    console.log('🔍 Moving fields to the correct last page (page 2)...');
    ...
    // Move fields to page 2 (last page, 0-based indexing)
    console.log('🔧 Moving fields to page 2 (last page)...');

    const fieldData = {
      client_timestamp: Math.floor(Date.now() / 1000),
      fields: [
        {
          page_number: 2, // Page 2 is the last page (0-based indexing)
          type: 'signature',
    ...
    console.log('✅ Fields moved to page 2 (last page)');

    // Wait a moment for the fields to be processed
    await new Promise((resolve) => setTimeout(resolve, 2000));

    // Verify the fields were moved to page 2
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      console.log('✅ Verification - Fields on page 2 (last page):');
      ...
      // Extract the corrected coordinates for page 2
      console.log('\n📋 Corrected field coordinates for page 2 (last page):');
      console.log('======================================================');
      ...
      console.log('❌ No fields found after moving to page 2');
      return null;
    }
  } catch (error) {
...
// Run the script
moveFieldsToCorrectPage().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Fields moved to page 2 (last page) successfully!');
    console.log(
      'The signature and date fields are now on the last page of the document.'
    );
  } else {
    console.log('\n❌ Failed to move fields to page 2');
  }
});
