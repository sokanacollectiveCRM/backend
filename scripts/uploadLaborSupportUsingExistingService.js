// Upload Labor Support Contract using the existing working SignNow service
// This uses the existing working SignNow service that was already set up

const SignNowService = require('../src/services/signNowService');
const fs = require('fs');
const path = require('path');

async function uploadLaborSupportUsingExistingService() {
  try {
    console.log('🚀 UPLOADING LABOR SUPPORT CONTRACT USING EXISTING SIGNNOW SERVICE\n');

    // 1️⃣ Initialize the existing SignNow service
...
// Run the upload
uploadLaborSupportUsingExistingService().catch(console.error);
