...
async function moveFieldsToPage3() {
  try {
    console.log('🔍 Moving fields to page 3 (last page)...');
    ...
    // Move fields to page 3 (last page) with proper positioning
    console.log('🔧 Moving fields to page 3...');

    const fieldData = {
      client_timestamp: Math.floor(Date.now() / 1000),
      fields: [
        {
          page_number: 3, // Move to page 3 (last page)
          type: 'signature',
    ...
    console.log('✅ Fields moved to page 3');

    // Wait a moment for the fields to be processed
    await new Promise((resolve) => setTimeout(resolve, 2000));

    // Verify the fields were moved to page 3
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      console.log('✅ Verification - Fields on page 3:');
      ...
      // Extract the corrected coordinates for page 3
      console.log('\n📋 Corrected field coordinates for page 3:');
      console.log('==========================================');
      ...
      console.log('❌ No fields found after moving to page 3');
      return null;
    }
  } catch (error) {
...
// Run the script
moveFieldsToPage3().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Fields moved to page 3 successfully!');
    console.log(
      'The signature and date fields are now on the last page of the document.'
    );
  } else {
    console.log('\n❌ Failed to move fields to page 3');
  }
});
