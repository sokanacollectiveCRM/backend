...
async function useSupabaseTemplateAndUpload() {
  try {
    console.log(
      '🔍 Using Supabase template, replacing placeholders, and uploading DOCX directly...'
    );
    ...
    if (downloadError) {
      console.error(
        '❌ Error downloading template from Supabase:',
        downloadError
      );
      return null;
    }

    console.log('✅ Template downloaded from Supabase');
    ...
    const templateVariables = {
      ...
      client_initials: contractData.clientName
        .split(' ')
        .map((n) => n[0])
        .join(''),
      client_signature: '',
...
// Run the script
useSupabaseTemplateAndUpload().then((result) => {
  if (result) {
    console.log(
      '\n🎉 Supabase template with replaced placeholders uploaded successfully!'
    );
    console.log('✅ Template downloaded from Supabase Storage');
    ...
    console.log(
      '\n❌ Failed to upload Supabase template with replaced placeholders'
    );
  }
});
