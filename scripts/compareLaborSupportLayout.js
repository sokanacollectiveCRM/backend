// Compare Layout Preservation for Labor Support Template
...
async function compareLaborSupportLayout() {
  try {
    console.log(
      '🔍 Comparing Layout Preservation for Labor Support Template...\n'
    );
    ...
    const generatedDocx = path.join(
      process.cwd(),
      'generated',
      'labor-support-contract-1761254135234.docx'
    );
    ...
    console.log('\n📊 File Status:');
    console.log(
      `   ✅ Original Template: ${filesExist.template ? 'EXISTS' : 'NOT FOUND'}`
    );
    console.log(
      `   ✅ Generated DOCX: ${filesExist.generatedDocx ? 'EXISTS' : 'NOT FOUND'}`
    );
    console.log(
      `   ✅ Generated PDF: ${filesExist.generatedPdf ? 'EXISTS' : 'NOT FOUND'}`
    );

    if (!filesExist.generatedPdf) {
      throw new Error(
        'Generated PDF not found. Please run the generation script first.'
      );
    }
    ...
    console.log('\n📋 Comparison Results:');
    console.log('1. ✅ Original Labor Support template converted to PDF');
    console.log('2. ✅ Generated Labor Support contract converted to PDF');
    console.log('3. ✅ Both PDFs opened for visual comparison');

    console.log('\n💡 What to Look For:');
    console.log('   📄 Check if all placeholders were replaced correctly');
    console.log(
      '   📄 Verify that formatting, logos, and styling are preserved'
    );
    console.log('   📄 Ensure that text positioning is accurate');
    console.log(
      '   📄 Confirm that the layout looks identical to the original'
    );
...
// Run the comparison
compareLaborSupportLayout().catch(console.error);
