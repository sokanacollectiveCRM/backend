...
async function fineTuneFieldPositions() {
  try {
    console.log('🔍 Fine-tuning field positions...');
    ...
    // Fine-tune positions: date up a tad, signature up a little more
    console.log('🔧 Fine-tuning field positions...');
      ...
      fields: [
        {
          ...
          y: 290, // Move date up just a tad (was 300)
          label: 'Date',
        },
        {
          page_number: 2, // Last page
          ...
          y: 280, // Move signature up a little more (was 300)
        },
      ],
    };
    ...
    console.log('✅ Fields fine-tuned');

    // Wait a moment for the fields to be processed
    await new Promise((resolve) => setTimeout(resolve, 2000));

    // Verify the fields were fine-tuned
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      console.log('✅ Verification - Fields fine-tuned:');
      ...
      // Extract the fine-tuned coordinates
      console.log('\n📋 Fine-tuned field coordinates:');
      console.log('=================================');
      ...
      console.log('❌ No fields found after fine-tuning');
      return null;
    }
  } catch (error) {
...
// Run the script
fineTuneFieldPositions().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Fields fine-tuned successfully!');
    console.log(
      'Date field moved up a tad, signature field moved up a little more for better alignment.'
    );
  } else {
    console.log('\n❌ Failed to fine-tune field positions');
  }
});
