...
async function moveSignatureUp() {
  try {
    console.log(
      '🔍 Moving signature field up to align with "Client Signature" text...'
    );
    ...
    console.log('🔧 Moving signature field up to align with text...');
    ...
    console.log('✅ Signature field moved up to align with text');
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      console.log('✅ Verification - Signature field moved up:');
      ...
      console.log('\n📋 Updated field coordinates (signature aligned):');
      console.log('================================================');
      ...
      console.log('❌ No fields found after moving signature up');
      return null;
    }
  } catch (error) {
...
moveSignatureUp().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Signature field moved up successfully!');
    console.log(
      'The signature field is now aligned with the "Client Signature" text.'
    );
  } else {
    console.log('\n❌ Failed to move signature field up');
  }
});
