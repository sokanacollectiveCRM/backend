...
async function switchPositionsAndAlign() {
  try {
    console.log('🔍 Switching field positions and aligning with text...');
    ...
    // Switch positions and move up slightly to align with text
    console.log('🔧 Switching positions and aligning with text...');
      ...
      fields: [
        {
          ...
          height: 25,
          width: 150,
          x: 100, // Move Date to LEFT side (was 350)
          y: 300, // Move up slightly (was 325) to align with "Date:" text
          label: 'Date',
        },
        {
          page_number: 2, // Last page
          ...
          height: 50,
          width: 200,
          x: 350, // Move Signature to RIGHT side (was 100)
          y: 300, // Move up slightly (was 325) to align with "Client Signature" text
        },
      ],
    };
    ...
    console.log('✅ Fields switched positions and aligned with text');
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      console.log('✅ Verification - Fields switched and aligned:');
      ...
      console.log(
        '\n📋 Updated field coordinates (switched positions and aligned):'
      );
      console.log(
        '=============================================================='
      );
      ...
      console.log('❌ No fields found after switching positions');
      return null;
    }
  } catch (error) {
...
// Run the script
switchPositionsAndAlign().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Fields switched positions and aligned successfully!');
    console.log(
      'Date field is now on the left, Signature field is on the right, and both are aligned with their respective text labels.'
    );
  } else {
    console.log('\n❌ Failed to switch positions and align fields');
  }
});
