...
async function addDateAndSignatureFields() {
  try {
    console.log('🔍 Adding date and signature fields with new positions...');
    ...
    // Add date and signature fields with the new positions we extracted
    console.log('🔧 Adding date and signature fields with new positions...');
    ...
    console.log('✅ Date and signature fields added with new positions');
    ...
    if (verifyResponse.data.fields && verifyResponse.data.fields.length > 0) {
      ...
      // Extract the coordinates for use in contract generation
      console.log('\n📋 Updated field coordinates for contract generation:');
      console.log('=====================================================');
...
// Run the script
addDateAndSignatureFields().then((coordinates) => {
  if (coordinates) {
    console.log('\n🎉 Date and signature fields added successfully!');
    console.log(
      'You can now use these updated coordinates in your contract generation process.'
    );
  } else {
    console.log('\n❌ Failed to add date and signature fields');
  }
});
