...
// This generates a Labor Support contract and converts it to PDF while preserving layout
...
async function testLaborSupportLayoutPreservation() {
  try {
    console.log('🚀 Testing Layout Preservation with Labor Support Template...\n');

    // 1️⃣ Download the Labor Support template from Supabase
    const templateFileName = 'Labor Support Agreement for Service.docx.pdf';
    ...
    const templatePath = path.join(process.cwd(), 'generated', 'labor-support-template.docx');
    ...
    console.log('\n📋 Next Steps:');
    console.log('1. ✅ Labor Support contract generated with perfect layout preservation');
    ...
    console.error('❌ Error generating Labor Support contract with PDF:', error);
    throw error;
  }
}

// Run the generation
testLaborSupportLayoutPreservation().catch(console.error);
