...
describe('Request Endpoint Tests', () => {
    let app;
    let requestFormController;
    let requestFormService;
    let requestFormRepository;
    let emailService;
    const mockFormData = {
        ...
        pronouns: types_1.Pronouns.SHE_HER,
        pronouns_other: '',
        // Step 2: Home Details
        address: '123 Main St',
        city: 'Anytown',
        state: types_1.STATE.CA,
        zip_code: '90210',
        home_phone: '555-987-6543',
        home_type: types_1.HomeType.HOUSE,
        home_access: 'Front door accessible',
        pets: '2 dogs, 1 cat',
        // Step 3: Family Members
        relationship_status: types_1.RelationshipStatus.MARRIED,
        first_name: 'John',
        last_name: 'Doe',
        middle_name: 'Michael',
        ...
        // Step 6: Payment Info
        annual_income: types_1.IncomeLevel.FROM_45000_TO_64999,
        service_needed: types_1.ServiceTypes.LABOR_SUPPORT,
        ...
        number_of_babies: 1,
        baby_name: 'Baby Doe',
        provider_type: types_1.ProviderType.OB,
        pregnancy_number: 2,
        ...
        services_interested: [types_1.ServiceTypes.LABOR_SUPPORT, types_1.ServiceTypes.POSTPARTUM_SUPPORT],
        service_support_details: 'Need help with breastfeeding',
        // Step 10: Client Demographics
        race_ethnicity: 'Caucasian',
        primary_language: 'English',
        client_age_range: types_1.ClientAgeRange.AGE_25_34,
    ...
    beforeEach(() => {
        ...
        requestFormRepository = new requestFormRepository_1.RequestFormRepository({});
        requestFormService = new RequestFormService_1.RequestFormService(requestFormRepository);
        requestFormController = new requestFormController_1.RequestFormController(requestFormService);
        emailService = new emailService_1.NodemailerService();
        // Create Express app for testing
        app = (0, express_1.default)();
        app.use(express_1.default.json());
        // Mount the request route
        app.post('/requestService/requestSubmission', (req, res) => requestFormController.createForm(req, res));
    });
    describe('POST /requestService/requestSubmission', () => {
        ...
        it('should handle service errors gracefully', async () => {
            ...
            jest.spyOn(requestFormService, 'newForm').mockRejectedValue(new Error('Database connection failed'));
            const response = await (0, supertest_1.default)(app)
                .post('/requestService/requestSubmission')
                .send(mockFormData)
                .expect(400);
        ...
        it('should handle email sending errors without blocking form submission', async () => {
            ...
            nodemailer.createTransport().sendMail.mockRejectedValue(new Error('Email service unavailable'));
            const response = await (0, supertest_1.default)(app)
                .post('/requestService/requestSubmission')
                .send(mockFormData)
                .expect(200);
    ...
    describe('RequestFormService Tests', () => {
        it('should validate required fields correctly', async () => {
            ...
            await expect(requestFormService.newForm(invalidData)).rejects.toThrow('Missing required fields: first name and last name');
        });
        it('should validate email format correctly', async () => {
            ...
            await expect(requestFormService.newForm(invalidEmailData)).rejects.toThrow('Valid email is required');
        });
        it('should validate phone number format correctly', async () => {
            ...
            await expect(requestFormService.newForm(invalidPhoneData)).rejects.toThrow('Invalid phone number format');
        });
        it('should validate zip code format correctly', async () => {
            ...
            await expect(requestFormService.newForm(invalidZipData)).rejects.toThrow('Invalid zip code format');
        });
        it('should validate complete address is provided', async () => {
            ...
            await expect(requestFormService.newForm(incompleteAddressData)).rejects.toThrow('Complete address is required');
        });
        it('should validate service_needed is provided', async () => {
            ...
            await expect(requestFormService.newForm(missingServiceData)).rejects.toThrow('Missing required field: service_needed');
        });
        it('should successfully process valid form data', async () => {
            // Mock the repository to return success
            jest.spyOn(requestFormRepository, 'saveData').mockResolvedValue(mockFormData);
            const result = await requestFormService.newForm(mockFormData);
            expect(result).toBeDefined();
            expect(requestFormRepository.saveData).toHaveBeenCalledWith(expect.objectContaining({
                firstname: 'Jane',
                lastname: 'Doe',
                email: 'jane.doe@example.com',
                service_needed: types_1.ServiceTypes.LABOR_SUPPORT
    ...
    describe('RequestFormRepository Tests', () => {
        it('should save form data to database successfully', async () => {
            ...
            const repository = new requestFormRepository_1.RequestFormRepository(mockSupabaseClient);
        ...
        it('should handle database errors gracefully', async () => {
            ...
            const repository = new requestFormRepository_1.RequestFormRepository(mockSupabaseClient);
            await expect(repository.saveData(mockFormData)).rejects.toThrow('Database insertion failed: Database connection failed');
        });
    ...
    describe('Email Service Tests', () => {
        it('should send email with correct configuration', async () => {
            ...
            await emailService.sendEmail('test@example.com', 'Test Subject', 'Test text content', '<p>Test HTML content</p>');
        ...
        it('should handle email sending errors', async () => {
            ...
            const emailService = new emailService_1.NodemailerService();
            emailService.transporter = mockTransporter;
            await expect(emailService.sendEmail('test@example.com', 'Test Subject', 'Test content')).rejects.toThrow('Failed to send email: SMTP error');
        });
    });
});
