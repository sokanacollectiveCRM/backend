...
exports.docusignService = exports.DocuSignService = void 0;
...
class DocuSignService {
    constructor() {
        ...
        this.apiClient = new docusign_esign_1.default.ApiClient();
        this.apiClient.setBasePath(`${this.basePath}/restapi`);
        // Set token file path
        this.tokenFilePath = path_1.default.join(process.cwd(), '.docusign-token.json');
        // Load access token from file if available
        this.loadAccessToken();
    }
    loadAccessToken() {
        try {
            if (fs_1.default.existsSync(this.tokenFilePath)) {
                const tokenData = JSON.parse(fs_1.default.readFileSync(this.tokenFilePath, 'utf8'));
                const now = Date.now();
                ...
                if (tokenData.expires_at && now < tokenData.expires_at) {
                    ...
                    fs_1.default.unlinkSync(this.tokenFilePath);
                }
            }
        }
        catch (error) {
    ...
    saveAccessToken(token, expiresIn) {
        try {
            ...
            fs_1.default.writeFileSync(this.tokenFilePath, JSON.stringify(tokenData, null, 2));
    ...
    async authenticate() {
        try {
            ...
            try {
                ...
                const jwtResult = await this.apiClient.requestJWTUserToken(this.clientId, this.userId, ['signature', 'impersonation'], '', // No private key needed for this approach
                3600);
    ...
    setAccessToken(token, expiresIn = 28800) {
    ...
    async createEnvelopeFromTemplate(templateId, recipientEmail, recipientName, fields, documentName) {
        try {
            await this.authenticate();
            ...
            const envelopesApi = new docusign_esign_1.default.EnvelopesApi(this.apiClient);
    ...
    async createEnvelopeWithPrefill(templateId, client, fields, options = {}) {
        try {
    ...
    async inspectTemplateFields(templateId) {
        try {
            await this.authenticate();
            console.log('🔍 Inspecting template fields for template:', templateId);
            // Make real API call to inspect template
            const templatesApi = new docusign_esign_1.default.TemplatesApi(this.apiClient);
    ...
    async generateContractFromTemplate(templateId, client, fields, options = {}) {
        try {
            await this.authenticate();
            ...
            const templatePath = path_1.default.join(process.cwd(), 'docs', 'Agreement for Postpartum Doula Services (1).docx');
            const originalTemplateBuffer = fs_1.default.readFileSync(templatePath);
    ...
    async processTemplateDocument(templateId, documentId, fields) {
        try {
            ...
            const processedBuffer = await documentProcessor_1.documentProcessor.processTemplate(fields);
            // Save processed document for debugging
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const filename = `contract-${timestamp}.docx`;
            await documentProcessor_1.documentProcessor.saveProcessedDocument(fields, filename);
            // Convert to base64
...
DocuSignService.storedCodeVerifier = null;
// Static storage for access token to persist across server restarts
DocuSignService.storedAccessToken = null;
// Export singleton instance
exports.docusignService = new DocuSignService();
