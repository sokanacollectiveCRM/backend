...
class SupabaseAuthService {
    ...
    async signup(email, password, firstname, lastname) {
        ...
        if (error) {
            throw new errors_1.AuthenticationError(`Authentication error: ${error.message}`);
        }
        if (!data.user) {
            throw new errors_1.AuthenticationError('User creation failed for unknown reasons');
        }
        const user = await this.userRepository.findByEmail(email);
        if (!user) {
            // This shouldn’t happen, but just in case
            await this.supabaseClient.auth.admin.deleteUser(data.user.id);
            throw new errors_1.AuthorizationError("Signup not allowed — not approved.");
        }
    ...
    async login(email, password) {
        ...
        if (error) {
            throw new errors_1.AuthenticationError('Authentication error: Sign in failed from Supabase');
        }
