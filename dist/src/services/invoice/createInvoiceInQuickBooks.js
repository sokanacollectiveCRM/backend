...
async function createInvoiceInQuickBooks(payload) {
    ...
    const { Invoice } = await (0, qboClient_1.qboRequest)('/invoice?minorversion=65', {
        method: 'POST',
        body: JSON.stringify(payload)
    });
    // Fetch the invoice again with the payment link
    const { Invoice: InvoiceWithLink } = await (0, qboClient_1.qboRequest)(`/invoice/${Invoice.Id}?minorversion=65&include=invoiceLink`, {
        method: 'GET'
    });
