...
async function convertToPdf(docxBuffer) {
    const cloudConvert = new cloudconvert_1.default(process.env.CLOUDCONVERT_API_KEY);
    try {
        ...
        const exportTask = completedJob.tasks.find((t) => t.name === 'export' && t.status === 'finished');
