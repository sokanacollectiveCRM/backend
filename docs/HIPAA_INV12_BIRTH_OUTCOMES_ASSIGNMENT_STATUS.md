...
- `updateClientBirthOutcomes` calls `canAccessSensitive` (admin or assigned
  doula).
...
- CRM save flows call `PUT /clients/:id/birth-outcomes` (dropdowns/checkboxes
  only).
...
- DB column `phi_clients.birth_outcomes` may still exist for historical rows; it
  is no longer writable via API or shown in CRM. Admin CSV export may still
  include the column until a separate export cleanup.
- `canAccessSensitive` assignment lookup uses Supabase
  `assignments.status = 'active'`; Cloud SQL assignment parity is tracked under
