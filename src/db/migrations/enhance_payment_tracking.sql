-- Enhance payment tracking system for contracts
-- This adds comprehensive payment scheduling and tracking

-- Step 1: Enhance the contract_payments table with more detailed tracking
...
ALTER TABLE contract_payments
ADD COLUMN IF NOT EXISTS overdue_notice_sent_at TIMESTAMP WITH TIME ZONE;

-- Step 2: Create payment schedules table for recurring payments
...
-- Step 3: Create payment reminders table
CREATE TABLE IF NOT EXISTS payment_reminders (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  payment_id UUID REFERENCES contract_payments(id) ON DELETE CASCADE,
  reminder_type VARCHAR(50) NOT NULL, -- 'due_soon', 'overdue', 'final_notice'
...
-- Step 5: Add foreign key constraint for payment_schedule_id
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 7: Create functions for payment management
...
BEGIN
  ...
  IF p_deposit_amount > 0 THEN
    INSERT INTO contract_payments (
      ...
      due_date, payment_number, total_payments, status
    ) VALUES (
      p_contract_id, v_schedule_id, 'deposit', p_deposit_amount,
      v_payment_date, 0, p_number_of_installments + 1, 'pending'
    );
    v_payment_number := 1;
  END IF;
  ...
  FOR i IN 1..p_number_of_installments LOOP
    ...
    INSERT INTO contract_payments (
      ...
      v_payment_date, v_payment_number, p_number_of_installments + CASE WHEN p_deposit_amount > 0 THEN 1 ELSE 0 END, 'pending'
    );
...
-- Function to get payment summary for a contract
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(cp.amount) FILTER (WHERE cp.status IN ('pending', 'failed') AND cp.due_date < CURRENT_DATE), 0) as overdue_amount,
    ...
    COUNT(cp.id) FILTER (WHERE cp.status IN ('pending', 'failed') AND cp.due_date < CURRENT_DATE) as overdue_count
...
CREATE OR REPLACE FUNCTION get_upcoming_payments(days_ahead INTEGER DEFAULT 7)
RETURNS TABLE (
  payment_id UUID,
  contract_id UUID,
  client_name TEXT,
...
BEGIN
  ...
  LEFT JOIN payment_schedules ps ON cp.payment_schedule_id = ps.id
  WHERE cp.status IN ('pending', 'failed')
  AND cp.due_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '1 day' * days_ahead
  ORDER BY cp.due_date ASC;
END;
$$ LANGUAGE plpgsql;
...
SELECT
  ...
  COUNT(cp.id) FILTER (WHERE cp.status IN ('pending', 'failed') AND cp.due_date < CURRENT_DATE) as overdue_payments,
  COALESCE(SUM(cp.amount) FILTER (WHERE cp.status = 'succeeded'), 0) as total_paid,
  COALESCE(SUM(cp.amount) FILTER (WHERE cp.status IN ('pending', 'failed')), 0) as total_due,
  MIN(cp.due_date) FILTER (WHERE cp.status IN ('pending', 'failed')) as next_payment_due
FROM contracts c
...
COMMENT ON TABLE payment_reminders IS 'Payment reminders and notifications tracking';
...
-- Step 10: Insert sample payment schedules for existing contracts
...
SELECT
  'Payment Tracking System Enhanced!' as status,
  ...
  (SELECT COUNT(*) FROM contract_payments WHERE due_date < CURRENT_DATE AND status IN ('pending', 'failed')) as overdue_payments;
