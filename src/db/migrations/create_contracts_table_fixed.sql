...
-- Step 2: Create main contracts table (this will replace the existing one)
-- First, let's backup the existing contracts table if it exists
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contracts' AND table_schema = 'public') THEN
    ...
    DROP TABLE IF EXISTS contracts_old_backup CASCADE;
    CREATE TABLE contracts_old_backup AS
    SELECT *, NOW() as backup_created_at FROM contracts;

    RAISE NOTICE 'Backed up existing contracts table to contracts_old_backup';
  END IF;
END $$;

-- Drop existing contracts table and recreate with new structure
DROP TABLE IF EXISTS contracts CASCADE;

-- Create new contracts table with proper client_info integration
...
-- Step 4: Create contract payments table (drop and recreate to ensure clean structure)
...
CREATE INDEX idx_contracts_generated_by ON contracts(generated_by);
CREATE INDEX idx_contracts_created_at ON contracts(created_at);
CREATE INDEX idx_contracts_signnow_document_id ON contracts(signnow_document_id);

-- SignNow integration indexes
CREATE INDEX idx_contract_signnow_contract_id ON contract_signnow_integration(contract_id);
CREATE INDEX idx_contract_signnow_document_id ON contract_signnow_integration(signnow_document_id);
CREATE INDEX idx_contract_signnow_status ON contract_signnow_integration(status);
...
-- Step 11: Migrate data from backup if it exists
DO $$
DECLARE
  backup_count INTEGER;
BEGIN
  -- Check if we have backup data to migrate
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contracts_old_backup' AND table_schema = 'public') THEN
    SELECT COUNT(*) INTO backup_count FROM contracts_old_backup;

    IF backup_count > 0 THEN
      -- Insert contracts from backup, trying to link to client_info
      ...
      SELECT
        ...
        COALESCE(ci.id, gen_random_uuid()) as client_id, -- Try to link by email
        'Migrated Contract' as template_name,
        c.deposit_amount::text as fee,
      ...
      FROM contracts_old_backup c
      LEFT JOIN client_info ci ON ci.email = c.client_email
      ON CONFLICT (id) DO NOTHING;

      RAISE NOTICE 'Migrated % contracts from backup', backup_count;
    END IF;
  END IF;
END $$;
