...
class ClientUseCase {
  ...
  async updateClientStatus(clientId, status, userId) {
    try {
      ...
      const currentClient =
        await this.clientRepository.findClientLiteById(clientId);
  ...
  async updateClientProfile(clientId, fieldsToUpdate, userId) {
    try {
      ...
      const currentClient =
        await this.clientRepository.findClientDetailedById(clientId);
      // Update the client
      ...
      const changedFields = [];
      if (
        fieldsToUpdate.user?.firstname !== undefined &&
        fieldsToUpdate.user.firstname !== currentClient.user.firstname
      ) {
        changedFields.push('firstname');
      }
      if (
        fieldsToUpdate.user?.lastname !== undefined &&
        fieldsToUpdate.user.lastname !== currentClient.user.lastname
      ) {
        changedFields.push('lastname');
      }
      if (
        fieldsToUpdate.user?.email !== undefined &&
        fieldsToUpdate.user.email !== currentClient.user.email
      ) {
        changedFields.push('email');
      }
      if (
        fieldsToUpdate.serviceNeeded !== undefined &&
        fieldsToUpdate.serviceNeeded !== currentClient.serviceNeeded
      ) {
