...
class AuthUseCase {
  ...
  async login(email, password) {
    ...
    try {
      ...
      if (!existingUser) {
        throw new errors_1.AuthorizationError(
          'Invalid credentials. Please try again or contact the office.'
        );
      }
  ...
  async handleOAuthCallback(code) {
    ...
    try {
      ...
      if (!user) {
        throw new errors_1.AuthorizationError(
          'You are not authorized to sign in. Please email the office if the issue persists.'
        );
      }
      return { session, user };
    } catch (error) {
  ...
  async handleToken(accessToken) {
    ...
    try {
      ...
      throw new errors_1.AuthenticationError(
        `Token handling error: ${error.message}`
      );
    }
  }
  //
  ...
  async handlePasswordRecovery(tokenHash, type) {
    ...
    try {
      ...
      throw new errors_1.AuthenticationError(
        `Failed to process password recovery: ${error.message}`
      );
    }
  }
  //
