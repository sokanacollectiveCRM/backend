...
class StripePaymentService {
    async ensureStripeCustomer(customerId) {
        ...
        if (customerData.stripe_customer_id) {
            try {
                await stripe_1.stripe.customers.retrieve(customerData.stripe_customer_id);
    ...
    async chargeCard({ customerId, amount, description }) {
        ...
        const { data: allPaymentMethods, error: allError } = await supabase_1.default
            .from('payment_methods')
            .select('*')
            .eq('customer_id', customerId);
        ...
        try {
            ...
            const { error: chargeError } = await supabase_1.default.from('charges').insert({
                customer_id: customerId,
    ...
    async updateCard({ customerId, cardToken, paymentMethodId }) {
        ...
        try {
            ...
            const { data: existingPaymentMethod, error: lookupError } = await supabase_1.default
                .from('payment_methods')
            ...
            await stripe_1.stripe.paymentMethods.detach(existingPaymentMethod.stripe_payment_method_id);
