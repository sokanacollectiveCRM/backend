...
class StripePaymentService {
  async ensureStripeCustomer(customerId) {
    console.log(
      `Ensuring Stripe customer exists for customer ID: ${customerId}`
    );
    ...
    if (customerData.stripe_customer_id) {
      try {
        await stripe_1.stripe.customers.retrieve(
        ...
        console.log(
          'Stripe customer ID exists in DB but not in Stripe, creating new one'
        );
  ...
  async saveCard({ customerId, cardToken }) {
    ...
    try {
      ...
      console.log(
        'Creating payment method from token and attaching to customer'
      );
      const paymentMethod = await stripe_1.stripe.paymentMethods.create({
        type: 'card',
      ...
      await stripe_1.stripe.paymentMethods.attach(paymentMethod.id, {
      ...
      const { data: insertResult, error } = await supabase_1.default
        .from('payment_methods')
  ...
  async chargeCard({ customerId, amount, description }) {
    ...
    const { data: allPaymentMethods, error: allError } =
      await supabase_1.default
        .from('payment_methods')
        .select('*')
        .eq('customer_id', customerId);
    ...
    const { data: paymentMethod, error } = await supabase_1.default
      .from('payment_methods')
    ...
    try {
      ...
      const { error: chargeError } = await supabase_1.default
        .from('charges')
        .insert({
          customer_id: customerId,
  ...
  async updateCard({ customerId, cardToken, paymentMethodId }) {
    console.log('Starting updateCard process for customer:', customerId);
    // Ensure customer exists in Stripe
    const stripeCustomerId = await this.ensureStripeCustomer(customerId);
    try {
      // Verify the payment method belongs to this customer
      const { data: existingPaymentMethod, error: lookupError } =
        await supabase_1.default
          .from('payment_methods')
      ...
      const newPaymentMethod = await stripe_1.stripe.paymentMethods.create({
        type: 'card',
        card: { token: cardToken },
      });
      ...
      await stripe_1.stripe.paymentMethods.attach(newPaymentMethod.id, {
        customer: stripeCustomerId,
      });
      ...
      await stripe_1.stripe.paymentMethods.detach(
        existingPaymentMethod.stripe_payment_method_id
      );
  ...
  async getPaymentMethods(customerId) {
    ...
    try {
      ...
      const { data: paymentMethods, error } = await supabase_1.default
        .from('payment_methods')
        .select(
          'id, stripe_payment_method_id, card_last4, card_brand, card_exp_month, card_exp_year, is_default, created_at'
        )
        .eq('customer_id', customerId)
      ...
      console.log(
        `Found ${paymentMethods?.length || 0} payment methods for customer`
      );
  ...
  async getCustomersWithStripeId() {
    ...
    try {
      ...
      const { data: customers, error } = await supabase_1.default
        .from('customers')
