...
class SignNowService {
  ...
  async addFieldsToDocument(documentId, fields) {
    try {
      ...
      console.error(
        'Error adding fields to document:',
        status,
        body || error.message
      );
      if (status === 404) {
        throw new Error(
          'SignNow returned 404. Possible causes: wrong environment/host (prod vs eval), documentId not found in this account, or incorrect base URL.'
        );
      }
      throw new Error(
        `Failed to add fields: ${body?.error || body?.message || error.message}`
      );
    }
  }
    ...
    try {
      ...
      for (const invite of existingInvites) {
        if (invite.email !== clientEmail) {
          try {
            await axios.delete(
              `${this.baseURL}/document/${documentId}/fieldinvite/${invite.id}`,
              {
                headers: this.getAuthHeaders(),
              }
            );
            ...
            console.warn(
              `Failed to cancel invitation for ${invite.email}:`,
              err.message
            );
          }
        }
      ...
      const role = roles.find(
        (r) => r.name === 'Sokana Collective' || r.name === 'Client'
      );
      if (!role) {
      ...
      console.error(
        'Error creating SignNow invitation:',
        error.response?.data || error.message
      );
      throw new Error(
        `Failed to create signing invitation: ${error.response?.data?.error || error.message}`
      );
    }
  }
  ...
  async createInvitationClientPartner(documentId, client, partner, options) {
    ...
    try {
      ...
      try {
        ...
        const existingInvites = docData.field_invites || [];
        console.log(
          'Found existing invites:',
          existingInvites.map((i) => ({
            id: i.id,
            email: i.email,
            status: i.status,
          }))
        ...
        const hasFulfilledInvite = existingInvites.some(
          (invite) =>
            invite.status === 'fulfilled' &&
            invite.email.toLowerCase() === client.email.toLowerCase()
        );

        if (hasFulfilledInvite) {
          throw new Error(
            'This document has already been signed by this client. Please create a new document copy for additional signatures.'
          );
        }
        ...
        if (hasPendingInvite) {
          throw new Error(
            'There is already a pending invitation for this client. Please wait for it to be signed or cancelled.'
          );
        }

        // If we get here, we can proceed with creating a new invitation
        console.log(
          'No existing invites found, proceeding with new invitation'
        );
      } catch (err) {
      ...
      const fetchRoles = async () => {
        try {
          ...
          const roles = Array.from(
            new Set(
              fields
                .map((f) => (typeof f.role === 'string' ? f.role.trim() : ''))
                .filter((r) => r)
            )
          );
          return roles;
      ...
      const recipientRole =
        fields.find((f) => f.role?.startsWith('Recipient'))?.role ||
        'Recipient 1';
      ...
      try {
        ...
        return {
          success: true,
          invite: data,
          rolesUsed: { clientRole, partnerRole },
          note: tryCustom ? 'Sent with custom subject/message' : undefined,
        };
      } catch (err) {
        const errs = err.response?.data?.errors;
        const planBlock =
          Array.isArray(errs) && errs.some((e) => Number(e.code) === 65582);
        if (planBlock && tryCustom) {
          ...
          return {
            ...
            invite: data,
            rolesUsed: { clientRole, partnerRole },
            note: 'Sent without custom subject/message due to plan limits',
          };
        }
        const roleMissing =
          Array.isArray(errs) && errs.some((e) => Number(e.code) === 65536);
        if (roleMissing) {
          throw new Error(
            `Document roles are ${JSON.stringify(rolesOnDoc)}; tried clientRole="${clientRole}" partnerRole="${partnerRole}".`
          );
        }
        throw err;
      }
  ...
  async getDocument(documentId) {
    try {
      ...
      throw new Error(
        `Failed to get document: ${error.response?.data?.error || error.message}`
      );
    }
  }
  ...
  async downloadDocument(documentId) {
    try {
      ...
      throw new Error(
        `Failed to download document: ${error.response?.data?.error || error.message}`
      );
    }
  }
  ...
  async getInvitationStatus(documentId) {
    try {
      ...
      console.error(
        'Error getting invitation status:',
        error.response?.data || error.message
      );
      throw new Error(
        `Failed to get invitation status: ${error.response?.data?.error || error.message}`
      );
    }
  }
  ...
  async processContractSigning(
    ...
    try {
      ...
      const invite = await this.createSigningInvitation(
        upload.documentId,
        clientEmail,
        clientName,
        contractId
  ...
  async testAuthentication() {
    try {
      ...
      throw new Error(
        `Authentication failed: ${JSON.stringify(errorDetails, null, 2)}`
      );
    }
  }
  ...
  async deleteEmbeddedInvite(documentId, inviteId) {
    await axios.delete(
      `${this.baseURL}/v2/documents/${documentId}/embedded-invites/${inviteId}`,
      {
        headers: this.getAuthHeaders(),
      }
    );
  ...
  async createEmbeddedInviteLink(documentId, signer, opts) {
    try {
      ...
      const order =
        typeof opts.order === 'number' && isFinite(opts.order) ? opts.order : 1;
      const expiresIn =
        typeof opts.expiresIn === 'number' && isFinite(opts.expiresIn)
          ? opts.expiresIn
          : 60;
      ...
      const method =
        typeof opts.auth_method === 'string' &&
        allowedAuth.includes(opts.auth_method.toLowerCase())
      ...
      if (
        (!rolesFromDoc || rolesFromDoc.length === 0) &&
        doc.signing_steps &&
        Array.isArray(doc.signing_steps.steps)
      ) {
        for (const step of doc.signing_steps.steps) {
          if (step && Array.isArray(step.roles))
            rolesFromDoc = rolesFromDoc.concat(step.roles);
        }
      }
      ...
      if (!match) {
        ...
        throw new Error(
          `Role "${roleName}" not found on document. Roles: ${JSON.stringify(available)}`
        );
      }
      ...
      const createInvite = async () => {
          ...
          invites: [
            {
              ...
              order,
              auth_method: method,
              ...(method === 'password' && opts.auth_password
        ...
        const d = res?.data || {};
        if (d.data && Array.isArray(d.data) && d.data[0]?.id)
          return d.data[0].id;
        if (d.invites && Array.isArray(d.invites) && d.invites[0]?.id)
          return d.invites[0].id;
        if (d.id) return d.id;
      ...
      try {
        ...
        const errs = err?.response?.data?.errors;
        const alreadyExists =
          Array.isArray(errs) && errs.some((e) => Number(e.code) === 19004002);
        ...
        const byRole = existing.filter(
          (it) => String(it.role_id) === String(match.id)
        );
        let candidate =
          byRole.find((it) => it.email && it.email === desiredEmail) ||
          byRole[0] ||
          existing[0];

        if (!candidate) {
          throw new Error(
            'An embedded invite exists but could not resolve an invite to reuse.'
          );
        }
        ...
        for (const inv of existing) {
          try {
            ...
            console.warn(
              'Delete embedded invite failed:',
              inv.id,
              dErr?.response?.data || dErr.message
            );
          }
        }
      ...
      console.error(
        'createEmbeddedInviteLink failed:',
