...
async function handleAuthCallback(callbackUrl) {
  ...
  const realmId =
    json.realmId ?? new url_1.URL(callbackUrl).searchParams.get('realmId');
  if (!realmId) {
...
async function isConnected() {
  ...
  if (isExpired) {
    ...
    const { getValidAccessToken } = await Promise.resolve().then(() =>
      __importStar(require('../../utils/tokenUtils'))
    );
