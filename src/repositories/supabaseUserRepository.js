...
class SupabaseUserRepository {
    ...
    try {
      ...
      const result = await Promise.all(
        hoursData.map(async (entry) => {
          ...
          if (!doulaData)
            throw new Error(
              `Doula with the ID ${entry.doula_id} not found, inside getAllHours()`
            );
          if (!clientData) {
  ...
  async addNewHours(doula_id, client_id, start_time, end_time, note) {
    ...
    if (note != '') {
      ...
      if (noteError) {
        throw new Error(
          `The note field is nonempty but failed to add note, ${noteError.message}`
        );
      }
    }
    return hoursData[0];
