...
export class SupabaseClientRepository {
  ...
  async updateClientOperational(
    ...
    if (dropped.length > 0) {
      console.warn(
        '[SupabaseClientRepo] Dropped non-operational keys from update:',
        dropped
      );
    }
    ...
    if (error) {
      ...
      throw new Error(
        `Failed to update client operational fields: ${error.message}`
      );
    }
  ...
  async updateClient(clientId: string, fieldsToUpdate: any): Promise<Client> {
    ...
    if (fieldsToUpdate.user?.firstname !== undefined)
      updateData.first_name = fieldsToUpdate.user.firstname;
    if (fieldsToUpdate.user?.lastname !== undefined)
      updateData.last_name = fieldsToUpdate.user.lastname;
    if (fieldsToUpdate.user?.first_name !== undefined)
      updateData.first_name = fieldsToUpdate.user.first_name;
    if (fieldsToUpdate.user?.last_name !== undefined)
      updateData.last_name = fieldsToUpdate.user.last_name;
    ...
    if (fieldsToUpdate.firstname !== undefined)
      updateData.first_name = fieldsToUpdate.firstname;
    if (fieldsToUpdate.lastname !== undefined)
      updateData.last_name = fieldsToUpdate.lastname;
    ...
    if (fieldsToUpdate.preferred_contact_method !== undefined)
      updateData.preferred_contact_method =
        fieldsToUpdate.preferred_contact_method;
    if (fieldsToUpdate.preferred_name !== undefined)
      updateData.preferred_name = fieldsToUpdate.preferred_name;
    if (fieldsToUpdate.payment_method !== undefined)
      updateData.payment_method = fieldsToUpdate.payment_method; // Add this field
    if (fieldsToUpdate.home_type !== undefined)
      updateData.home_type = fieldsToUpdate.home_type;
    if (fieldsToUpdate.services_interested !== undefined)
      updateData.services_interested = fieldsToUpdate.services_interested;
    if (fieldsToUpdate.health_notes !== undefined)
      updateData.health_notes = fieldsToUpdate.health_notes;
    if (fieldsToUpdate.baby_sex !== undefined)
      updateData.baby_sex = fieldsToUpdate.baby_sex;
    if (fieldsToUpdate.baby_name !== undefined)
      updateData.baby_name = fieldsToUpdate.baby_name;
    if (fieldsToUpdate.birth_hospital !== undefined)
      updateData.birth_hospital = fieldsToUpdate.birth_hospital;
    if (fieldsToUpdate.birth_location !== undefined)
      updateData.birth_location = fieldsToUpdate.birth_location;
    if (fieldsToUpdate.number_of_babies !== undefined)
      updateData.number_of_babies = fieldsToUpdate.number_of_babies;
    if (fieldsToUpdate.provider_type !== undefined)
      updateData.provider_type = fieldsToUpdate.provider_type;
    if (fieldsToUpdate.pregnancy_number !== undefined)
      updateData.pregnancy_number = fieldsToUpdate.pregnancy_number;
    if (fieldsToUpdate.had_previous_pregnancies !== undefined)
      updateData.had_previous_pregnancies =
        fieldsToUpdate.had_previous_pregnancies;
    if (fieldsToUpdate.previous_pregnancies_count !== undefined)
      updateData.previous_pregnancies_count =
        fieldsToUpdate.previous_pregnancies_count;
    if (fieldsToUpdate.living_children_count !== undefined)
      updateData.living_children_count = fieldsToUpdate.living_children_count;
    if (fieldsToUpdate.past_pregnancy_experience !== undefined)
      updateData.past_pregnancy_experience =
        fieldsToUpdate.past_pregnancy_experience;
    if (fieldsToUpdate.service_support_details !== undefined)
      updateData.service_support_details =
        fieldsToUpdate.service_support_details;
    if (fieldsToUpdate.race_ethnicity !== undefined)
      updateData.race_ethnicity = fieldsToUpdate.race_ethnicity;
    if (fieldsToUpdate.primary_language !== undefined)
      updateData.primary_language = fieldsToUpdate.primary_language;
    if (fieldsToUpdate.client_age_range !== undefined)
      updateData.client_age_range = fieldsToUpdate.client_age_range;
    if (fieldsToUpdate.insurance !== undefined)
      updateData.insurance = fieldsToUpdate.insurance;
    if (fieldsToUpdate.demographics_multi !== undefined)
      updateData.demographics_multi = fieldsToUpdate.demographics_multi;
    if (fieldsToUpdate.pronouns_other !== undefined)
      updateData.pronouns_other = fieldsToUpdate.pronouns_other;
    if (fieldsToUpdate.home_phone !== undefined)
      updateData.home_phone = fieldsToUpdate.home_phone;
    if (fieldsToUpdate.home_access !== undefined)
      updateData.home_access = fieldsToUpdate.home_access;
    if (fieldsToUpdate.pets !== undefined)
      updateData.pets = fieldsToUpdate.pets;
    if (fieldsToUpdate.relationship_status !== undefined)
      updateData.relationship_status = fieldsToUpdate.relationship_status;
    if (fieldsToUpdate.first_name !== undefined)
      updateData.first_name = fieldsToUpdate.first_name;
    if (fieldsToUpdate.last_name !== undefined)
      updateData.last_name = fieldsToUpdate.last_name;
    if (fieldsToUpdate.middle_name !== undefined)
      updateData.middle_name = fieldsToUpdate.middle_name;
    if (fieldsToUpdate.mobile_phone !== undefined)
      updateData.mobile_phone = fieldsToUpdate.mobile_phone;
    if (fieldsToUpdate.work_phone !== undefined)
      updateData.work_phone = fieldsToUpdate.work_phone;
    if (fieldsToUpdate.referral_source !== undefined)
      updateData.referral_source = fieldsToUpdate.referral_source;
    if (fieldsToUpdate.referral_name !== undefined)
      updateData.referral_name = fieldsToUpdate.referral_name;
    if (fieldsToUpdate.referral_email !== undefined)
      updateData.referral_email = fieldsToUpdate.referral_email;
    if (fieldsToUpdate.referral_source_other !== undefined)
      updateData.referral_source_other = fieldsToUpdate.referral_source_other;
    if (fieldsToUpdate.address !== undefined)
      updateData.address = fieldsToUpdate.address;
    if (fieldsToUpdate.city !== undefined)
      updateData.city = fieldsToUpdate.city;
    if (fieldsToUpdate.state !== undefined)
      updateData.state = fieldsToUpdate.state;
    if (fieldsToUpdate.country !== undefined)
      updateData.country = fieldsToUpdate.country;
    if (fieldsToUpdate.zip_code !== undefined)
      updateData.zip_code = fieldsToUpdate.zip_code;
    if (fieldsToUpdate.profile_picture !== undefined)
      updateData.profile_picture = fieldsToUpdate.profile_picture;
    if (fieldsToUpdate.account_status !== undefined)
      updateData.account_status = fieldsToUpdate.account_status;
    if (fieldsToUpdate.business !== undefined)
      updateData.business = fieldsToUpdate.business;
    ...
    const dropped = keysBefore.filter(
      (k) => !ALLOWED_CLIENT_INFO_UPDATE_COLUMNS.has(k)
    );
    if (dropped.length > 0) {
      console.warn(
        '[SupabaseClientRepository] Dropped unknown update keys (not in client_info whitelist):',
        dropped
      );
    }
    ...
    if (!data) {
      throw new Error(
        `No data returned after update for client ID: ${clientId}`
      );
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          console.warn(
            '[ClientRepo] assignments table not found — returning empty list for doula'
          );
          return [];
        }
        throw new Error(error.message);
  ...
  private mapToClient(data: any): Client {
    ...
    const user = this.mapToUser({
      ...
      preferred_contact_method:
        userRecord.preferred_contact_method || data.preferred_contact_method,
      ...
      services_interested:
        userRecord.services_interested || data.services_interested,
      ...
      had_previous_pregnancies:
        userRecord.had_previous_pregnancies || data.had_previous_pregnancies,
      previous_pregnancies_count:
        userRecord.previous_pregnancies_count ||
        data.previous_pregnancies_count,
      living_children_count:
        userRecord.living_children_count || data.living_children_count,
      past_pregnancy_experience:
        userRecord.past_pregnancy_experience || data.past_pregnancy_experience,
      service_support_details:
        userRecord.service_support_details || data.service_support_details,
      ...
      demographics_multi:
        userRecord.demographics_multi || data.demographics_multi,
      ...
      relationship_status:
        userRecord.relationship_status || data.relationship_status,
      ...
      referral_source_other:
        userRecord.referral_source_other || data.referral_source_other,
    });
